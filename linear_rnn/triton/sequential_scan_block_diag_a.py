...
def _sequential_scan_block_diag_a_fwd_kernel(
    ...
    x_ptr += (
        pid_batch * stride_x_batch + pid_block * stride_x_block
    )  # x: (batch, seq_len, num_blocks, m=block_dim)
    a_ptr += (
    ...
    a_ptrs = (
        a_ptr
        + offsets_dim_m[:, None] * stride_a_dim_m
        + offsets_dim_n[None, :] * stride_a_dim_n
    )
    ...
    for t in range(seq_len):
        ...
        for n in range(tl.cdiv(block_dim, BLOCK_SIZE_N)):
            a_t = tl.load(a_ptrs + n * BLOCK_SIZE_N * stride_a_dim_n).to(
                tl.float32
            )
            prev_ht = (
                ...
                if t == 0
                    ...
                    out_ptrs
                    - stride_out_len
                    + n * BLOCK_SIZE_N * stride_out_dim_m
                ).to(tl.float32)
            )
            acc += tl.sum(
                a_t * prev_ht[:, None], axis=1, keep_dims=False
            )  # accumulate sum along axis-n
...
class SequentialScanBlockDiagA(torch.autograd.Function):
    ...
    def forward(ctx: Any, x: torch.Tensor, a: torch.Tensor) -> torch.Tensor:
        ...
        print(
            f"x_elements_per_block={BLOCK_SIZE}, threads_per_block={num_warps * 32}"
        )

        assert dim % BLOCK_SIZE == 0
        assert (
            dim == num_blocks * block_dim
        ), f"{(num_blocks * block_dim)=} mismatches with {dim=}"
        assert is_power_of_2(
            BLOCK_SIZE
        ), f"{BLOCK_SIZE=} must be a power of two"

        x = rearrange(
            x, "... (n d) -> ... n d", n=num_blocks
        )  # (batch, seq_len, num_blocks, block_dim)
